...
import { broadcastMutationEvent, broadcastReactQueryEvent } from "./reactotron-helpers"

// fix types next release
type ReactotronReactQuery = (queryClientManager: any) => any

function reactotronReactQuery(queryClientManager: QueryClientManager): ReactotronReactQuery {
  return (reactotron: ReactotronCore) => {
    queryClientManager.subscribe(
      (event) => broadcastReactQueryEvent(reactotron, event),
      (event) => broadcastMutationEvent(reactotron, event)
    )
